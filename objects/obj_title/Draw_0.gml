...
if (start == 0) {
	...
	if (display == 1) {
		...
		draw_text_shadowed(camera_get_view_x(view_camera[0]) + camera_get_view_width(view_camera[0]) / 2, camera_get_view_y(view_camera[0]) + camera_get_view_height(view_camera[0]) / 2 + 48, "PRESIONA " + string(key_to_string(global.key[input.action_0])) + " PARA INICIAR!", c_black, c_white, 1, 1, 0.5, 1);
	}
...
//Otherwise if input has been given from the player, display menu
	...
	if (sub_menu == 2) {
		...
		//	draw_set_halign(fa_center);
		//	draw_text_shadowed(camera_get_view_x(view_camera[0]) + camera_get_view_width(view_camera[0]) / 2, camera_get_view_y(view_camera[0]) + camera_get_view_height(view_camera[0]) - 24, "PRESIONA UNA TECLA....", c_black, c_red, 1, 1, 0.5, 1);
		//	draw_set_halign(fa_left);
		//}
	}
...
draw_set_halign(fa_center);
draw_text_shadowed(camera_get_view_x(view_camera[0]) + camera_get_view_width(view_camera[0]) / 2, camera_get_view_y(view_camera[0]) + camera_get_view_height(view_camera[0]) - 24 + (yy / 2), string_upper("2023 - 2024 TESISTAS \n TECNICO SALESIANO"), c_black, c_white, 1, 1, 0.5, 1);
draw_set_halign(fa_left);